...
Our estimate of the odds ratio for solved homicides is `r balt_tidy %>%  mutate(estimate = round(exp(estimate), 3)) %>% select(estimate)`. We are 95% confident that the true odds ratio for solving homicides, comparing non-Whites to whites is (`r balt_tidy %>% mutate(conf.low = round(exp(conf.low), 3)) %>% select(conf.low)`, `r balt_tidy %>%  mutate(conf.high = round(exp(conf.high), 3)) %>% select(conf.high)`).
